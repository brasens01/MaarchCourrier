...
trait ExportSEDATrait
{
    public static function sendToRecordManagement(array $args)
    {
        ...
        if (empty($doctype['retention_rule']) || empty($doctype['retention_final_disposition'])) {
            ...
            if ($resource['binding'] === null && !in_array($doctype['action_current_use'], ['transfer', 'sae_copy'])) {
                return ['errors' => 'action_current_use is not transfer or sae_copy'];
            } elseif ($resource['binding'] === true && !in_array($bindingDocument['param_value_string'], ['transfer', 'sae_copy'])) {
                return ['errors' => 'binding document is not transfer or sae_copy'];
            } elseif ($resource['binding'] === false && !in_array($nonBindingDocument['param_value_string'], ['transfer', 'sae_copy'])) {
                return ['errors' => 'no binding document is not transfer or sae_copy'];
            }
